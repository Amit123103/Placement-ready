...
import { Timer, ArrowRight, ArrowLeft, CheckCircle2, Terminal, ChevronLeft } from "lucide-react";
...
export default function PreviewMockTestPage() {
  ...
  if (loading) return <DashboardLayout><div className="flex min-h-[50vh] items-center justify-center"><div className="w-10 h-10 border-4 border-primary border-t-transparent rounded-full animate-spin"></div></div></DashboardLayout>;
  if (!activeTest) return <DashboardLayout><div className="p-8 text-center">Test not found</div></DashboardLayout>;
    ...
    <DashboardLayout>
      <div className="flex items-center gap-4 mb-6">
        <Button variant="outline" size="icon" asChild>
          <Link href={`/mock-tests/${resolvedId}`}><ChevronLeft className="w-4 h-4" /></Link>
        </Button>
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Preview Test</h2>
          <p className="text-muted-foreground mt-1">This is how users will see {activeTest.title}</p>
        </div>
      </div>

      <div className="flex-1 bg-muted/20 relative overflow-hidden py-6 rounded-xl border border-border/50">
                  ...
                  <div className="md:col-span-3 bg-card/60 backdrop-blur-md p-8 rounded-2xl border border-border/50 shadow-md flex flex-col min-h-[500px]">
                    <div className="flex justify-between items-center mb-6">
                      ...
                      <Badge variant="outline" className="border-border">
                        {currentQ?.type?.toUpperCase() || "UNKNOWN"} • {currentQ?.marks || 1} Marks
                      </Badge>
                    </div>
            ...
            {testResult && activeTest && (
              ...
              >
                <div className="bg-card/80 backdrop-blur-md p-10 rounded-3xl border border-border/60 shadow-xl text-center space-y-6">
                  ...
                  <h2 className="text-3xl font-extrabold">Preview Completed!</h2>
                  <p className="text-muted-foreground text-lg">
                    This is the final score screen the user will see.
                  </p>
                  ...
                  <div className="pt-4">
                    <Button asChild className="rounded-xl">
                      <Link href={`/mock-tests/${resolvedId}`}>Back to Builder</Link>
                    </Button>
                  </div>
                </div>
              </motion.div>
