  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === "auth/email-already-in-use") {
        try {
          // Attempt to log them in automatically with the password they provided
          await signInWithEmail(email, password);
          // If successful, onAuthStateChanged will handle the redirect
        } catch (loginErr: any) {
          setError("This email is already registered. Please log in with your correct password.");
        }
      ...
      <Card className="w-full max-w-md glass-card">
        <CardHeader className="space-y-1">
          ...
          <CardDescription className="text-center">
            Sign up to start preparing for your dream placement.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Button variant="outline" className="w-full flex-1" onClick={signInWithGoogle}>
            ...
            <Button variant="outline" className="w-full flex-1" onClick={signInWithApple}>
